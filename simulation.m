 %% Ρομποτικός Χειριστής 3 βαθμών-Παραμέτροι
clear all;
close all;
l(1) = 5.0;
l(2) = 5.0; %% σε cm
...
dt = 0.001; %dt=1 msec
%%  Χώρος Εργασίας
Tf=10.0; % περίοδος της κίνησης
t=0:dt:Tf;
%xdA,xdB,ydA,ydB,zdA,zdB: αρχικ'ο και τελικό σημείο δράσης της επιθυμητής
%τροχιάς
xdA = -20.0;
xdB = 20.0;
ydA = -10.00;
...
% Υλοποίηση θέσης του τελικού σημείου δράσης 
...
%% ****** Κινηματική Προσομοίωση ******
disp('Kinematic Simulation ...'); %%
disp(' '); %%


Pez=zdB;
%Ομαλοποιούμε την κίνηση μέσω πολυωνιμικών συναρτήσεων παρεμβολής
...
s1=sin(qd(1,1)); % s1 και c1 σταθερά γιατί q1 σταθερό(δεν εξαρτάται από τα q2,q3)
c1=cos(qd(1,1)); 
...
ylabel('dqd3/dt (rad)');
xlabel('time t (sec)'); 
%% Διάγραμμα κίνησης του ρομποτικού χειριστή 
fig5 = figure;
axis([-30 30 -30 30 -40 40]) %% ορίζουμε τα όρια των 3 αξόνων
grid on
axis on
hold on
xlabel('x (cm)');
...
zlabel('z (cm)');
plot3(xd,yd,zd,'rs'); 
dtk=1000; %% πλοτάρουμε κατά dtk δείγματα για την εκτέλεση της προσομοίωσης
plot3([0],[0],[0],'o'); 
for tk=1:dtk:kmax,
pause(0.1); %% κάνουμε παύση του σχήματος για καλύτερη εποπτεία της διάταξης κάθε στιγμή
